...
bool update(float delta_time) {
  ...
  if (glfwGetKey(renderer::get_window(), GLFW_KEY_Q))
	  meshes["chaser"].get_transform().rotate(vec3(-1.0f,0.0f, 0.0f));
  if (glfwGetKey(renderer::get_window(), GLFW_KEY_E))
	  meshes["chaser"].get_transform().rotate(vec3(1.0f, 0.0f, 0.0f));
  // Use keyboard to move the target_mesh - WSAD
  if (glfwGetKey(renderer::get_window(), GLFW_KEY_W)) {
	  meshes["chaser"].get_transform().position += vec3(1.0f, 0.0f, 0.0f);
  }
  if (glfwGetKey(renderer::get_window(), GLFW_KEY_D)) {
	  meshes["chaser"].get_transform().position += vec3(0.0f, -1.0f, 0.0f);
  }
...
void main() {
  ...
  // Run application
  application.run();
>>>>>>> 3e8e89d09b606a55b8ccce5cf89b0cdadc263bc3
}